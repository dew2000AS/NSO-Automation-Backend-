...
public interface TmpReadingsRepository extends JpaRepository<TmpReadings, TmpReadingsId> {
    ...
    Optional<Date> findLatestReadingDateByAccNbrAndAreaCdAndBillCycle(@Param("accNbr") String accNbr, 
       ...
       /**
        * Get error statistics count for bar chart (unique accounts per error type)
        * Only includes readings from active bill cycles
        */
       @Query("SELECT t.errStat, COUNT(DISTINCT t.accNbr) as accountCount, COUNT(t) as totalErrors " +
              "FROM TmpReadings t " +
              "WHERE t.errStat IN (1, 2, 3, 4, 5, 6, 7) " +
              "AND t.addedBlcy IN (SELECT CAST(bc.billCycle AS string) FROM BillCycleConfig bc " +
              "                   WHERE bc.areaCode = :areaCd AND bc.cycleStat = 1) " +
              "GROUP BY t.errStat " +
              "ORDER BY t.errStat")
       List<Object[]> findErrorStatisticsByArea(@Param("areaCd") String areaCd);

       /**
        * Get error statistics count for all areas (Admin view)
        * Only includes readings from active bill cycles
        */
       @Query("SELECT t.errStat, COUNT(DISTINCT t.accNbr) as accountCount, COUNT(t) as totalErrors " +
              "FROM TmpReadings t " +
              ...
              "GROUP BY t.errStat " +
              "ORDER BY t.errStat")
       List<Object[]> findErrorStatisticsAllAreas();

       /**
        * Get detailed account information for a specific error type
        * Only includes readings from active bill cycles
        */
       @Query("SELECT t.accNbr, t.mtrType, t.rdngDate " +
              "FROM TmpReadings t " +
              "WHERE t.errStat = :errStat " +
              "AND t.areaCd = :areaCd " +
              "AND t.addedBlcy IN (SELECT CAST(bc.billCycle AS string) FROM BillCycleConfig bc " +
              "                   WHERE bc.areaCode = :areaCd AND bc.cycleStat = 1) " +
              "ORDER BY t.accNbr, t.mtrType")
       List<Object[]> findAccountsByErrorType(@Param("areaCd") String areaCd, @Param("errStat") Integer errStat);

       /**
        * Get detailed account information for a specific error type across all areas (Admin view)
        */
       @Query("SELECT t.accNbr, t.areaCd, t.mtrType, t.rdngDate " +
              "FROM TmpReadings t " +
              "WHERE t.errStat = :errStat " +
              "AND t.addedBlcy IN (SELECT CAST(bc.billCycle AS string) FROM BillCycleConfig bc " +
              "                   WHERE bc.areaCode = t.areaCd AND bc.cycleStat = 1) " +
              "ORDER BY t.areaCd, t.accNbr, t.mtrType")
       List<Object[]> findAccountsByErrorTypeAllAreas(@Param("errStat") Integer errStat);

       /**
        * Get all accounts with errors for an area (for total count)
        * Only includes readings from active bill cycles
        */
       @Query("SELECT COUNT(DISTINCT t.accNbr) " +
              "FROM TmpReadings t " +
              "WHERE t.errStat IN (1, 2, 3, 4, 5, 6, 7) " +
              "AND t.areaCd = :areaCd " +
              "AND t.addedBlcy IN (SELECT CAST(bc.billCycle AS string) FROM BillCycleConfig bc " +
              "                   WHERE bc.areaCode = :areaCd AND bc.cycleStat = 1)")
       Long countDistinctAccountsWithErrors(@Param("areaCd") String areaCd);

       /**
        * Get total error instances for an area
        * Only includes readings from active bill cycles
        */
       @Query("SELECT COUNT(t) " +
              ...
              "AND t.addedBlcy IN (SELECT CAST(bc.billCycle AS string) FROM BillCycleConfig bc " +
              "                   WHERE bc.areaCode = :areaCd AND bc.cycleStat = 1)")
       Long countTotalErrorInstances(@Param("areaCd") String areaCd);
}